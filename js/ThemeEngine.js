const ThemeEngine = {
	...
	setTheme: (mode, save = true) => { // mode: 'light', 'dark', 'system'
		...
		if (save) {
			localStorage.setItem(DB_KEYS.THEME_MODE, mode);
        ...
        const savedColor = (user && user.settings && user.settings.themeColor) || localStorage.getItem(DB_KEYS.THEME_COLOR);
        if (savedColor) {
    ...
    setThemeColor: (hex, save = true) => {
		...
		if (save) {
			localStorage.setItem(DB_KEYS.THEME_COLOR, hex);
			const user = window.DB && window.DB.ctx();
			if (user) {
				window.DB.updateUser(user.id, { 
	...
	toggle: () => {
		...
		if (user) {
			 currentSetting = (user.settings && user.settings.themeMode) || localStorage.getItem(DB_KEYS.THEME_MODE) || 'system';
		} else {
			 // Guest: read from localstorage (if they toggled before) or default 'light'
			 currentSetting = localStorage.getItem(DB_KEYS.THEME_MODE) || 'light';
		}
